...
class FeatureDataset:
    def __init__(self, name, ID, project_path):
        ...
        self.dff = pd.DataFrame({'ID': [ID] * len(starts), 'StartInSec': starts, 'EndInSec': ends})
    ...
    def add_disruptor_numbers(self):
        ...
        for s, e in zip(self.dff['StartInSec'], self.dff['EndInSec']):
            ...
            np_lst.append(sum('P' in l for l in x))
            na_lst.append(sum('IA' in l for l in x))
...
def create_long_formats():
    ...
    for i in range(len(data_lst)):
        ...
        data = FeatureDataset(name, ID, project_path)
