    ...
    for step, data in enumerate(train_loader):
        ...
        def test(loader, used_for, epoch):
            ...
            for data in loader:  # Iterate in batches over the training/test dataset.
                ...
                correct += int((pred == torch.tensor(data.y).long()).sum())  # Check against ground-truth labels.

                pred_lst += pred.tolist()
                true_lst += data.y.tolist()

            f1_score = f1_score(true_lst, pred_lst, average=None)#  average = 'macro', labels=[0,1,2])
            return correct / len(loader), f1_score  # Derive ratio of correct predictions.

        for epoch in range(1, nepoch):
            train()
            train_acc, train_f1 = test(train_dataset, 'train_set', epoch)
            test_acc, test_f1 = test(test_dataset, 'test_set', epoch)
            train_f1 = [np.round(f1,4) for f1 in train_f1]
            test_f1 = [np.round(f1, 4) for f1 in test_f1]
            print(f'Epoch: {epoch:03d}, Train Acc: {train_acc:.4f}, Test Acc: {test_acc:.4f}')
            #print(f'Epoch: {epoch:03d}, Train F1: {train_f1:.4f}, Test F1: {test_f1:.4f}')
            print(f'Epoch: {epoch:03d}, Train F1: {train_f1}, Test F1: {test_f1}')
            print('-----------')